...
export default function DashPosts() {
  ...
  useEffect(() => { //useEffect se llama cuando el componente se monta y cada vez que currentUser._id cambia
    const fetchPosts = async () => {
      try {
        const response = await fetch(`/api/post/getposts?userId=${currentUser._id}`);   //_id viene de la base de datos
        const data = await response.json();
       if(response.ok) {
          setUserPosts(data.posts); // data.posts viene del backend, es un array de posts,en postcontroller.js linea 40
    ...
    fetchPosts(); //se llama la funcion
  }, [currentUser._id])
  ...
  const handleShowMore = async () => {
    ...
    try {
      ...
      if(response.ok) {
        ...
        setUserPosts((prev) => [...prev, ...data.posts]); // data.posts viene del backend, es un array de posts,en postcontroller.js linea 40
        if(data.posts.length < 9) {
          setShowMore(false);
        }
      }
  ...
  const handleDeletePost = async () => {
      setShowModal(false);
      try {
          const response = await fetch(`/api/post/deletepost/${postIdDelete}/${currentUser._id}`, {
              method: 'DELETE',
          });
          const data = await response.json();
          if(!response.ok) {
                ...
                <Table.Body key={post._id} className="divide-y">
                    <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <Table.Cell>
                            <span onClick={()=> {
                              setShowModal(true);
                              setPostIdDelete(post._id); //postIdDelete es un estado que se crea arriba
